...
def get_time_to_wait():
    ...
    for prev_dir in sorted(prev_dirs, reverse=True):
        ...
        time_to_wait = (next_date - (datetime.datetime.now()-datetime.timedelta(days=7))).total_seconds()
        log(time_to_wait)
...
def run_once():
    dir_prefix = datetime.datetime.strftime(datetime.datetime.now() - datetime.timedelta(days=7), "%Y-%m-%d")
